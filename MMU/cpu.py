...
class CPU:
    ...
    def __init__(self, ram, os, num=0):
        ...
        self._ram = ram
        self._mmu = MMU(ram) # create the MMU object
        self._os = os
        self._debug = False
        ...
        while True:
            ...
            if self._debug:
                ...
                print("CPU {}: executing code at [{}]: {}".format(self._num, self._registers['pc'],
                                                          self._mmu.getMemValue(self._registers['pc'])))

            # Execute the next instruction.
            self.parse_instruction(self._mmu.getMemValue(self._registers['pc']))
    ...
    def _get_value_at(self, addr):
        ...
        or hex.'''
        addr = eval(addr[1:])
        return self._mmu.getMemValue(addr)
    ...
    def handle_mov(self, src, dst):
        ...
        if self.isregister(dst):
            ...
            if self.isregister(dst[1:]):
                self._mmu.setMemValue(self._registers[dst[1:]], srcval)
            else:
                print("Illegal instruction")
                return
        else:   # assume dst holds a literal value
            self._mmu.setMemValue(eval(dst), srcval)
    ...
    def handle_add(self, src, dst):
        ...
        if self.isregister(dst):
            ...
            if self.isregister(dst[1:]):
                self._mmu.setMemValue(self._registers[dst[1:]], self._mmu.getMemValue(dst[1:]) + srcval) 
            else:
                print("Illegal instruction")
                return
        else:   # assume dst holds a literal value
            self._mmu.setMemValue(eval(dst), self._mmu.getMemValue(eval(dst)) + srcval)
    ...
    def handle_sub(self, src, dst):
        ...
        if self.isregister(dst):
            ...
            if self.isregister(dst[1:]):
                self._mmu.setMemValue(self._registers[dst[1:]], self._mmu.getMemValue(self._registers[dst[1:]]) - srcval)
            else:
                print("Illegal instruction")
                return
        else:   # assume dst holds a literal value
