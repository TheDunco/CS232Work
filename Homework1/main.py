...
class Monitor:
    ...
    # @duncan added R batch instruction
        ...
        while True:
            try:
                ...
                if instr == '?':
                    ...
                    print("R <addr>: run the in batch mode starting from addr and ending at the first 0") # DOCUMENT THIS
                ...
                elif instr.upper().startswith('R '): # DOCUMENT THIS
                    print("Running batch mode")
                    startaddr = eval(instr.split()[1])
                    self._run_batch(startaddr)
    ...
    # @duncan added this instruction to run with the cpu in batch mode
    def _run_batch(self, addr):
        self._cpu = CPU(self._ram, calos.CalOS(), 0, self._debug)
        self._cpu.set_batch_mode(True)
        self._cpu.set_batch_address(addr)
        self._cpu.start()		# call run()
        self._cpu.join()		# wait for it to end
        self._cpu.set_batch_mode(False)
