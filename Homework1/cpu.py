...
class CPU(threading.Thread):
    ...
    # @duncan Set the starting address of the batch programs.
    def set_batch_address(self, address):
        self._batch_address = address

    # @ duncan Set the program into batch mode
    def set_batch_mode(self, mode):
        self._batch_mode = mode

    # @duncan Clear the non-pc registers
    ...
    # @duncan modified this function to run in batch mode
    def run(self):
        '''Overrides run() in thread.  Called by calling start().'''
        if not self._batch_mode:
            self._run_program()
        elif self._batch_mode:
            while self._ram[self._batch_address] != 0:
                # Set pc to next program
                self._registers['pc'] = self._ram[self._batch_address]
                self._run_program()
                self.clear_registers()
                # Move to next program in batch list
                self._batch_address += 1
